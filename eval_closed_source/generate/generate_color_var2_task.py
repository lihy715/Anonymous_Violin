...
def get_eval_config(model_type, save_root):
    ...
    # Define subfolder
    save_dir = os.path.join(save_root, f"{model_type}_color_var2")
...
def run_evaluation(model_type, save_root=None):
    """
    Main logic for VIOLIN Task Color Var-2 automation testing.
    """
    ...
    metadata_path = os.path.join(project_root, "benchmark/metadata/Task_Color_Var2_metadata.csv")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run VIOLIN Task Color Var-2 Tasks for Closed-Source Models.")
    parser.add_argument(
        '--model', 
        type=str, 
        required=True, 
