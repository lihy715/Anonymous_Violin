...
def get_eval_config(model_type, save_root):
    ...
    # Define subfolder: e.g., closed_source_results/gpt_variation_1
    save_dir = os.path.join(save_root, f"{model_type}_variation_1")
...
def run_evaluation(model_type, save_root=None):
    """
    Main logic for VIOLIN Variation-1 automation testing.
    """
    # Path derivation: script is at root/eval_closed_source/generate/, 
    # move up 3 levels to find project root.
    ...
    metadata_path = os.path.join(project_root, "benchmark/metadata/Variation_1_metadata.csv")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run VIOLIN Variation-1 Tasks for Closed-Source Models.")
    parser.add_argument(
        '--model', 
        type=str, 
        required=True, 
